...
/**
 * @author bob7l
 *     <p>Handles automatic triggers to entity removal.
 */
...
public class TriggerManager extends ClearlagModule {
  ...
  public void setEnabled() {
    ...
    if (config.getConfigurationSection("custom-trigger-removal.triggers") == null) {
      ...
      Util.warning(
          "custom-trigger-removal is enabled, yet doeesn't contain any triggers? Disabling...");
    ...
    for (String triggerKey :
        config.getConfigurationSection("custom-trigger-removal.triggers").getKeys(false)) {

      final CleanerTrigger trigger;

      CleanerHandler cleanerHandler = new CleanerHandler();

      final String cleanerType =
          config.getString("custom-trigger-removal.triggers." + triggerKey + ".trigger-type");
      ...
      if (trigger == null) {
        ...
        try {
          configHandler.setObjectConfigValues(
              trigger, "custom-trigger-removal.triggers." + triggerKey);
        ...
        for (String cleanerKey :
            config
                .getConfigurationSection("custom-trigger-removal.triggers." + triggerKey + ".jobs")
                .getKeys(false)) {
          ...
          if (module == null) {
            ...
            try {
              configHandler.setObjectConfigValues(
                  module, "custom-trigger-removal.triggers." + triggerKey + ".jobs." + cleanerKey);
              Clearlag.getInstance().getAutoWirer().wireObject(module);
            } catch (Exception e) {
              Util.warning(
                  "Failed to set config variables for job '"
                      + cleanerKey
                      + "', and trigger "
                      + triggerKey);
              e.printStackTrace();
            ...
            if (config.get(
                    "custom-trigger-removal.triggers."
                        + triggerKey
                        + ".jobs."
                        + cleanerKey
                        + ".warnings")
              ...
              try {
                configHandler.setObjectConfigValues(
                    module,
                    "custom-trigger-removal.triggers." + triggerKey + ".jobs." + cleanerKey);
                Clearlag.getInstance().getAutoWirer().wireObject(module);
              } catch (Exception e) {
                Util.warning(
                    "Failed to set config variables for warnings on job '"
                        + cleanerKey
                        + "', and trigger "
                        + triggerKey);
                e.printStackTrace();
        ...
        final BukkitTask runnableTask =
            new BukkitRunnable() {
              ...
              public void run() {
                ...
                Clearlag.getInstance(), trigger.getCheckFrequency(), trigger.getCheckFrequency());
  ...
  public void setDisabled() {
    ...
    for (BukkitTask task : tirggerTaskMap.values()) task.cancel();
