...
public class LogPurger extends TaskModule {
  ...
  public void run() {
    ...
    long time =
        new Date().getTime()
            - (86400000L * configHandler.getConfig().getLong("log-purger.days-old"));
    ...
    if (files != null) {
      ...
      for (File file : files) {
        if (file.isFile()
            ...
            && time > Util.parseTime(file.getName().replace(".log.gz", "")).getTime()) {
          file.delete();
          deleted++;
        }
      }
  ...
  public int startTask() {
    return Bukkit.getScheduler()
        .runTaskLaterAsynchronously(Clearlag.getInstance(), this, 0L)
        .getTaskId();
  }
}
