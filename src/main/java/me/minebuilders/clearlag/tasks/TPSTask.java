...
public class TPSTask extends TaskModule {
  ...
  protected int startTask() {
    ...
    if (configHandler.getConfig().getBoolean("settings.use-internal-tps")) {
      ...
      try {
        ...
        Util.warning(
            "Clearlag failed to use the internal TPS tracker during initialization. Reverted to estimation... ("
                + e.getMessage()
                + ")");
    ...
    return Bukkit.getScheduler()
        .scheduleSyncRepeatingTask(Clearlag.getInstance(), this, 120L, getInterval());
  }
  ...
  private class EstimatedTPSCalculator implements TPSCalculator {
    ...
    public void tick() {
      ...
      if (lasTimestamp != -1) {
        ...
        if (elaspedTime == 49 || elaspedTime == 51) elaspedTime = 50;
      ...
      if (++elapsedTicks % 20 == 0) {
        ...
        for (int tickLength : tickLengths) tickSum += tickLength;
  ...
  private class InternalTPSYoinker implements TPSCalculator {
    ...
    public InternalTPSYoinker() {
      // No initialization needed for modern API
    }
