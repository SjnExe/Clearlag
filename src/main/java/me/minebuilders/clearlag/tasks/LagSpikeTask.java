...
public class LagSpikeTask extends TaskModule {
    ...
    public void run() {
      ...
      if (elapsedTime >= minElapsedTime) {
        ...
        if (currentTick != frozenTick) {
          ...
          Util.warning(
              "Clearlag has detected a possible lag spike on tick #"
                  + currentTick
                  + " (Tick is currently at "
                  + elapsedTime
                  + " milliseconds)");
          ...
          if (stackTraceElements.length > 0) {
            ...
            for (StackTraceElement ste : stackTraceElements) Util.log(" > " + ste);
            ...
            && !stackTraceElements[0].toString().equals(frozenLine)) {
        ...
        Util.warning(
            "Thread '" + mainThread.getName() + "' is no longer stuck on tick #" + frozenTick);
        Util.warning("Estimated time spent on tick #" + frozenTick + ": " + lastElaspedTime);
        Util.warning(
            "Garbage collection time during tick: "
                + (getTotalGCCompleteTime() - lastGarbageCollectionTimeTotal));
      }
    }
  }
  ...
  protected int startTask() {
    ...
    timer.scheduleAtFixedRate(
        new ThreadWatcherTask(),
        50,
        configHandler.getConfig().getLong("lag-spike-helper.check-interval"));

    return Bukkit.getScheduler()
        .runTaskTimer(Clearlag.getInstance(), this, getInterval(), getInterval())
        .getTaskId();
  }
