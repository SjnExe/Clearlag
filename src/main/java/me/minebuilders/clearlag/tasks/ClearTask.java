...
public class ClearTask extends TaskModule {
  ...
  @ConfigModule private final AutoClear autoClear = new AutoClear();

  @ConfigValue private int autoremovalInterval;
  ...
  public void run() {
    ...
    if (broadcastWarning != null)
      broadcastHandler.broadcast(
          Util.cloneAndReplaceStringArr(
              broadcastWarning, "+remaining", "" + (autoremovalInterval - interval)));
      ...
      if (broadcastRemoval)
        broadcastHandler.broadcast(
            Util.cloneAndReplaceStringArr(
                broadcastMessage,
                "+RemoveAmount",
                String.valueOf(entityManager.removeEntities(autoClear))));
