...
public class TPSCheckTask extends TaskModule {
  ...
  @ConfigValue private List<String> commands;

  @ConfigValue private List<String> recoverCommands;

  @ConfigValue private double tpsTrigger;

  @ConfigValue private double tpsRecover;
  ...
  @ConfigValue private boolean broadcastEnabled;
  ...
  public void run() {
    ...
    if (tps <= tpsTrigger && isRecovered) {
      ...
      if (broadcastEnabled) Bukkit.broadcastMessage(triggerBroadcastMessage);

      try {
        for (String s : commands) Bukkit.dispatchCommand(Bukkit.getConsoleSender(), s);
      } catch (Exception e) {
      ...
      try {
        ...
        if (broadcastEnabled) Bukkit.broadcastMessage(recoverBroadcastMessage);

        for (String s : recoverCommands) Bukkit.dispatchCommand(Bukkit.getConsoleSender(), s);
      } catch (Exception e) {
        Util.warning("TPSCheckTask was unable to dispatch commands!");
      } finally {
        isRecovered = true;
  ...
  protected int startTask() {
    return Bukkit.getScheduler()
        .scheduleSyncRepeatingTask(Clearlag.getInstance(), this, 420, getInterval());
  }
}
