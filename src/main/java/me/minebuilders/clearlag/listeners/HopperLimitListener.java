...
public class HopperLimitListener extends EventModule implements Runnable {
  ...
  @ConfigValue private final int transferLimit = 6;

  @ConfigValue private final int checkInterval = 1;
  ...
  public void onHopper(InventoryMoveItemEvent event) {
    ...
    if (event.getSource().getHolder() instanceof Hopper) {
      ...
      if (transfers >= transferLimit) event.setCancelled(true);
      else ++transfers;
    ...
    schedulerID =
        Bukkit.getScheduler()
            .scheduleSyncRepeatingTask(
                Clearlag.getInstance(), this, checkInterval * 20L, checkInterval * 20L);
  }
