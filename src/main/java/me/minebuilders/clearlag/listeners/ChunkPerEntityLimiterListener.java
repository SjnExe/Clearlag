...
public class ChunkPerEntityLimiterListener extends EventModule {
    ...
    if (limit != null) {
      ...
      if (entities.length >= limit) {
        ...
        for (Entity e : entities)
          if (entity.getType() == e.getType() && entityLimits.getValue(e).equals(limit)) ++count;
  ...
  private class PassivePurger extends BukkitRunnable {
    ...
    public void run() {
      ...
      if (worldIndex >= worlds.size()) worldIndex = 0;
  ...
  public void setEnabled() {
    ...
    if (Clearlag.getInstance()
        .getConfig()
        .getBoolean("per-entity-chunk-entity-limiter.passive-cleaner.passive-cleaning-enabled"))
      passivePurger =
          new PassivePurger(
                  Clearlag.getInstance()
                      .getConfig()
                      .getInt("per-entity-chunk-entity-limiter.passive-cleaner.chunk-batch-size"))
              .runTaskTimer(
                  Clearlag.getInstance(),
                  Clearlag.getInstance()
                      .getConfig()
                      .getInt("per-entity-chunk-entity-limiter.passive-cleaner.check-interval"),
                  Clearlag.getInstance()
                      .getConfig()
                      .getInt("per-entity-chunk-entity-limiter.passive-cleaner.check-interval"));
  }
