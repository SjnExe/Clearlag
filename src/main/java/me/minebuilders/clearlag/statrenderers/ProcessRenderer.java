...
public class ProcessRenderer extends StatRenderer {
  ...
  public void draw(MapView mapView, MapCanvas mapCanvas, Player player) {
    ...
    if (totalTime != 0) {
      ...
      mapCanvas.drawText(
          5, 5, MinecraftFont.Font, "§44;Thread-Usage: §16;" + ((int) threadUsage) + "%");

      final double tickLength = Math.round((threadUsage * 0.5) * 10.0) / 10.0;

      mapCanvas.drawText(5, 17, MinecraftFont.Font, "§44;Tick-Length: §16;" + (tickLength) + "ms");

      mapCanvas.drawText(
          6, 29, MinecraftFont.Font, "§44;Memory: §16;" + RAMUtil.getUsedMemory() + "MB");
    }
  }
