      ...
      try {
        ...
        if (entityType != null) {
          ...
          try {
            ...
            final AttributeParser.Result result =
                attributeParser.getAttributesFromString(entityType, tokenizer);

            for (String nonApplicable : result.getNonApplicables())
              Util.warning(
                  "Invalid entity attribute path=\"" + path + "\" token=" + nonApplicable + "\"");
