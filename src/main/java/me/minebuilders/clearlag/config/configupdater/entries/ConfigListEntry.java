...
public class ConfigListEntry implements ConfigEntry {
  ...
  public void merge(ConfigEntry entry) {
    ...
    if (entry.getValue() instanceof List) values = (List<Object>) entry.getValue();
    else values = new ArrayList<>(1);
  }
  ...
  public void write(BufferedWriter writer, int tabs) throws IOException {
    ...
    for (int i = 0; i < tabs; ++i) tabLine.append(ConfigEntry.TAB);

    writer.write(tabLine + key + ":");

    if (values.isEmpty()) writer.write(" []");
    else {
