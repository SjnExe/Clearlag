...
public class ConfigHandler {
  ...
  public ConfigHandler() {
    ...
    try {
          ...
          .setFilter(
              new Filter() {
                ...
                public boolean isLoggable(LogRecord record) {
                  ...
                  if (record.getMessage().contains("ClearLag")) configLoadFailed.set(true);
      ...
      if (configLoadFailed.get()) {
        ...
        Util.warning(
            "Clearlag failed to load to your config. Clearlag will now attempt to repair your config by force-updating it...");
        ...
        try {
          ...
          Util.warning(
              "Clearlag failed to force-update your config. Using the default config - please repair your configuration using https://yaml-online-parser.appspot.com/");
          throw e;
        }
      }
    ...
    if (!new File(Clearlag.getInstance().getDataFolder(), "config.yml").exists()) {
      ...
      if (config.getBoolean("config-updater.force-update") && !isConfigUpdated()) {
        ...
        try {
          ...
          Util.warning(
              "Clearlag was unable to update your configuration and was forced to rename your current config and create a new one!");
          Util.warning(
              "Please run your config through a parser to check for errors: http://yaml-online-parser.appspot.com/");
  ...
  public boolean isConfigUpdated() {
    return (config.isSet("config-version")
        && Util.isInteger(config.getString("config-version"))
        && (config.getInt("config-version") >= version));
  }
  ...
  public void setObjectConfigValues(Object object, String path)
      throws Exception { // Todo: Apply config setting to objects too
    ...
    while (clazz != null && clazz != Object.class && clazz != Module.class) {
      ...
      for (Field field : clazz.getDeclaredFields()) {
        ...
        if (configValue != null) {
          ...
          Object ob =
              cd.getValue(
                  configValue.path().length() <= 1
                      ? path + "." + fieldToConfigValue(field)
                      : configValue.path());
          ...
          field.set(
              object,
              (cd instanceof PrimitiveCV
                  ? ReflectionUtil.castPrimitedValues(field.getType(), ob)
                  : ob));
        }
  ...
  public static boolean containsReloadableFields(Object ob) {
    for (Field field : ob.getClass().getDeclaredFields()) {
      if (field.isAnnotationPresent(ConfigValue.class)
          || field.isAnnotationPresent(ConfigModule.class)) {
        return true;
      }
    }
    return false;
  ...
  private void renameCurrentConfig(String newName) {
    ...
    final File newf =
        new File(Clearlag.getInstance().getDataFolder().getAbsolutePath(), "config.yml");
    final File oldf = new File(Clearlag.getInstance().getDataFolder().getAbsolutePath(), newName);

    if (oldf.isFile()) oldf.delete();
  ...
  private void updateConfig() throws Exception {
    ...
    configUpdater.setUpdatingFromConfig(
        new FileInputStream(Clearlag.getInstance().getDataFolder() + "/config.yml"));
    ...
    Util.log(
        "Successfully updated config to v"
            + Clearlag.getInstance().getDescription().getVersion()
            + "!");
  }
}
