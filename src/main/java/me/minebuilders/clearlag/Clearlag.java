...
public class Clearlag extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    if (versionAdapter == null) {
      ...
      Util.warning(
          "Clearlag failed to find a valid version adapter for your version ("
              + Util.getRawBukkitVersion()
              + ")");
  ...
  private void startModules() {
    ...
    for (Module mod : modules) {
      ...
      if (configPath == null
          || (getConfig().get(configPath.path() + ".enabled") == null
              || getConfig().getBoolean(configPath.path() + ".enabled"))) {
        mod.setEnabled();
      }
    }
