    ...
    while (tk.hasMoreTokens()) {
      ...
      if (tok.startsWith("name=\"") || tok.startsWith("hasMeta=\"")) {
        ...
        if (t == EntityType.ITEM) {
          ...
          String input = tok.startsWith("id=") ? tok.substring(3) : tok.substring(9);
          Material mat = null;
          ...
          if (Util.isInteger(input)) {
            ...
            Util.warning(
                "ID's are no longer usable in your Spigot version - Please change \""
                    + id
                    + "\" into the item/block's name");
