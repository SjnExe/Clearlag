...
public class TickSamplerCmd extends CommandModule {
      ...
      callback =
          (samples) -> {
            final IntSummaryStatistics stats =
                samples.stream().mapToInt((x) -> x).summaryStatistics();
            ...
            lang.sendMessage(
                "print",
                sender,
                (Util.getChatColorByNumberLength(stats.getMax(), 40, 50).toString()
                    + stats.getMax()),
                (Util.getChatColorByNumberLength(stats.getMin(), 40, 50).toString()
                    + stats.getMin()),
                (Util.getChatColorByNumberLength((int) Math.round(stats.getAverage()), 40, 50)
                        .toString()
  ...
  private static class TimingThread extends Thread {
    ...
    public TimingThread(
        Thread watchingThread, Callback<Collection<Integer>> callback, int sampleTickCycles) {
