  ...
  protected void run(Player player, String[] args) throws WrongCommandArgumentException {
    ...
    if (!Util.isInteger(args[0]) || !Util.isInteger(args[1])) {
      throw new WrongCommandArgumentException(
          lang.getMessage("invalidinteger"), (Util.isInteger(args[0]) ? args[1] : args[0]));
    }
    ...
    player.teleport(
        new Location(world, x, world.getHighestBlockYAt(new Location(world, x, y, z)), z));
