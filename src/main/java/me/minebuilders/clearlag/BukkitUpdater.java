...
public class BukkitUpdater implements Runnable {
  ...
  private boolean updateAvailable() throws Exception {
    ...
    Util.log(
        "Checking for updates compatible with your bukkit version ["
            + Util.getBukkitVersion()
            + "]...");
    ...
    return (versionToInt(Clearlag.getInstance().getDescription().getVersion())
        < versionToInt(newversion));
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (updateAvailable()) {
        ...
        if (!Bukkit.getUpdateFolderFile().exists()) Bukkit.getUpdateFolderFile().mkdir();
        ...
        try (BufferedInputStream in = new BufferedInputStream(conn.getInputStream());
            FileOutputStream fout =
                new FileOutputStream(
                    Bukkit.getUpdateFolderFile().getAbsolutePath() + "/" + file.getName())) {
