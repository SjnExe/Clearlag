...
public class LanguageLoader {
    ...
    while (clazz != null && clazz != Object.class && clazz != Module.class) {
      ...
      for (Field field : clazz.getDeclaredFields()) {
        ...
        if (field.isAnnotationPresent(LanguageValue.class)) {
          ...
          if (message == null)
            message =
                new BasicMessage(broadcastHandler, ChatColor.RED + "!Missing-From-Language-File!");
  ...
  public Message getMessageByKey(String key) {
    ...
    if (message == null) {
      ...
      if (fallbackLanguageMap != null) message = fallbackLanguageMap.get(key);
      else
        message =
            new BasicMessage(broadcastHandler, ChatColor.RED + "!Missing-From-Language-File!");
    }
  ...
  private Map<String, Message> loadLanguage(InputStream in) throws Exception {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      String[] replaceValues =
          lineParts[0]
              .substring(lineParts[0].indexOf("(") + 1, lineParts[0].indexOf(")"))
              .split(",");

      if (replaceValues.length == 1 && replaceValues[0].isEmpty()) replaceValues = new String[0];

      final String message = Util.color(lineParts[1]);

      // Replace the representation of a replaced value with a shortend version. Small
      // optimization...
      ...
      if (message.trim().equals("{")) {
        ...
        insertingMessage =
            new MessageBlock(
                broadcastHandler, currentMessageBlock.toArray(new String[0]), replaceValues);
