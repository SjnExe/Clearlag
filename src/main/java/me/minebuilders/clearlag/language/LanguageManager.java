...
public class LanguageManager extends ClearlagModule {
  ...
  public void setEnabled() {
    ...
    try {
      ...
      languageLoader.setLanguageMap(
          Clearlag.class.getResource("/languages/" + desiredLanguage).openStream());

    } catch (Exception e) {

      Util.warning(
          "Clearlag FAILED to find your desired language file '"
              + desiredLanguage
              + "'. Defaulting to English...");

      try {
        languageLoader.setLanguageMap(
            Clearlag.class.getResource("/languages/English.lang").openStream());
      } catch (Exception ex) {
        ex.printStackTrace();
      }
    }
